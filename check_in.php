...
$sql = "SELECT c.first_name, c.last_name, check_in.id, check_in.checked_in FROM contacts c LEFT JOIN check_in ON c.id=check_in.contact_id WHERE check_date = $date ORDER BY checked_in ASC";
$result=mysql_query($sql) OR die("The information could not be retrieved from the database.");
$check_ins=array();	
while($row=mysql_fetch_array($result,MYSQL_BOTH)){array_push($check_ins, $row);}
mysql_close();
